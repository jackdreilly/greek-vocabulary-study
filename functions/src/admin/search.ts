/**
 * Mirror of web/src/lib/search.ts so admin tool queries match what the
 * search box matches in-app. Greeklish input like "ksipnaw" matches "ξυπνάω".
 *
 * Keep these rules in sync with web/src/lib/search.ts.
 */
...
export function transliterate(text: unknown): string {
  if (!text) return "";
  let s = String(text);
...
export function textMatchesSearch(text: unknown, query: unknown): boolean {
