...
export const generateVocabFlow = getAI().defineFlow(
  ...
  async (input) => {
    ...
    const { output } = await getAI().generate({
      ...
      prompt: `Generate ${input.count} Modern Greek vocabulary entries.

Course: ${input.courseTitle}
Course description: ${input.courseDescription}
Course source request: ${input.courseSourcePrompt}
Lesson: ${input.lessonTitle}
Lesson description: ${input.lessonDescription}
Lesson source request: ${input.lessonSourcePrompt}
